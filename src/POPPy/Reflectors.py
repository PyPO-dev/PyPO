...
######################### REFLECTOR INFO #########################\n""".format(self.reflectorType, self.reflectorId,
    ...
    def translateGrid(self, offTrans, units='mm', save=True):
        ...
        if save:
            ...
            Txyz = np.diag(np.ones(4))
            Txyz[0,-1] = offTrans[0] * conv
            Txyz[1,-1] = offTrans[1] * conv
            Txyz[2,-1] = offTrans[2] * conv
            Txyz[3,-1] = 1

            self.matListPoints.append(Txyz)
    ...
    def rotateGrid(self, offRot, radians=False, save=True, invert=False):
        ...
        gridRot, Rxyz = MatRotate.MatRotate(offRot, [self.grid_x, self.grid_y, self.grid_z], self.cRot,
                                            radians=radians, invert=invert, matOut=True, affine=True)

        self.grid_x = gridRot[0]
        self.grid_y = gridRot[1]
        self.grid_z = gridRot[2]

        grid_nRot, Rxyz_v = MatRotate.MatRotate(offRot, [self.grid_nx, self.grid_ny, self.grid_nz],
                                        self.cRot, vecRot=True, radians=radians,
                                        invert=invert, matOut=True, affine=True)
        ...
        if save:
            ...
            self.matListPoints.append(Rxyz)
            self.matListNorms.append(Rxyz_v)
    ...
    def getFullTransform(self, fwd=True):
        mat = np.eye(4)
        mat_v = np.eye(4)

        if fwd:
            for transf in self.matListPoints:
                mat = np.matmul(transf, mat)

            for transf_v in self.matListNorms:
                mat_v = np.matmul(transf_v, mat_v)

        else:
            for transf in reversed(self.matListPoints):
                mat = np.matmul(np.linalg.inv(transf), mat)

            for transf_v in reversed(self.matListNorms):
                mat_v = np.matmul(np.linalg.inv(transf_v), mat_v)
