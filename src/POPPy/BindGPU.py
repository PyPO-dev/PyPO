...
"""
def calcJM_GPUd(source, target, currents, k, epsilon, t_direction, nThreads=256):
    lib = loadGPUlib()
    ...
    lib.callKernel_JM.argtypes = [ctypes.POINTER(c2Bundle),
    ...
    lib.callKernel_JM.restype = None

    xyzs, xyzt, nxyzt, area, rJs, iJs, rMs, iMs = currentConv(source, target, currents, ctypes.c_double)
    ...
    res = c2Bundle()
    allocate_c2Bundle(res, target.shape[0] * target.shape[1], ctypes.c_double)

    lib.callKernel_JM(ctypes.byref(res),
...
def calcEH_GPUd(source, target, currents, k, epsilon, t_direction, nThreads=256):
    lib = loadGPUlib()
    ...
    # Define arg and return types
    lib.callKernel_EH.argtypes = [ctypes.POINTER(c2Bundle),
    ...
    lib.callKernel_EH.restype = None
    ...
    lib.callKernel_EH(ctypes.byref(res),
                            xyzt[0], xyzt[1], xyzt[2],
                            xyzs[0], xyzs[1], xyzs[2],
                            rJs[0], rJs[1], rJs[2],
                            iJs[0], iJs[1], iJs[2],
...
def calcJMEH_GPUd(source, target, currents, k, epsilon, t_direction, nThreads=256):
    lib = loadGPUlib()
    ...
    # Define arg and return types
    lib.callKernel_JMEH.argtypes = [ctypes.POINTER(c4Bundle),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
    ...
    lib.callKernel_JMEH.restype = None
    ...
    # We pass reference to struct to c-function.
    res = c4Bundle()
    allocate_c4Bundle(res, target.shape[0] * target.shape[1], ctypes.c_double)

    lib.callKernel_JMEH(ctypes.byref(res),
                            xyzt[0], xyzt[1], xyzt[2],
                            xyzs[0], xyzs[1], xyzs[2],
                            nxyzt[0], nxyzt[1], nxyzt[2],
                            rJs[0], rJs[1], rJs[2],
...
def calcEHP_CPUd(source, target, currents, k, numThreads, epsilon, t_direction):
    ...
    # Define arg and return types
    lib.callKernel_EHP.argtypes = [ctypes.POINTER(c2rBundle),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
                                       ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
    ...
    lib.callKernel_EHP.restype = None
    ...
    # We pass reference to struct to c-function.
    res = c2rBundle()
    allocate_c2rBundle(res, target.shape[0] * target.shape[1], ctypes.c_double)

    lib.callKernel_EHP(ctypes.byref(res),
                            xyzt[0], xyzt[1], xyzt[2],
                            xyzs[0], xyzs[1], xyzs[2],
                            nxyzt[0], nxyzt[1], nxyzt[2],
                            rJs[0], rJs[1], rJs[2],
