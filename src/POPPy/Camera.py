...
######################### CAMERA INFO #########################\n""".format(self.center[0], self.center[1], self.center[2])
    ...
    def setGrid(self, lims_x, lims_y, gridsize, gmode):
        ...
        if gmode == 'xy':
            ...
            # Store uv increments, for efficiency calculations
            self.du = du
            self.dv = dv
        ...
        self.grid_x = grid_x
        self.grid_y = grid_y
        self.grid_z = np.zeros(self.grid_x.shape)
        ...
        self.translateGrid(offTrans=self.center, init=True)
    ...
    def translateGrid(self, offTrans, units='mm', save=True, init=False):
        """
        (PUBLIC)
