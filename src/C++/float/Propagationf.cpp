...
Propagationf::Propagationf(float k, int numThreads, int gridsize_s, int gridsize_t, float thres, float epsilon, float t_direction, int toPrint)
{
    M_PIf = 3.1415926;
    
    C_L = 2.9979245; // mm s^-1
    MU_0 = 1.2566370e-3; // kg mm s^-2 A^-2
    EPS_VAC = 1 / (MU_0 * C_L*C_L);
    ZETA_0_INV = 1 / (C_L * MU_0);
    
    std::complex<float> j(0., 1.);
    std::complex<float> z0(0., 0.);
    this->j = j;
    this->z0 = z0;
...
void Propagationf::propagateBeam(int start, int stop,
                                ...
                                const std::vector<float> &source_area)
{
    ...
    std::complex<float> e_dot_p_r_perp;    // E-field - perpendicular reflected POI polarization vector dot product
    std::complex<float> e_dot_p_r_parr;    // E-field - parallel reflected POI polarization vector dot product
    
    // Arrays of floats
    std::array<float, 3> S_i_norm;         // Normalized incoming Poynting vector
    std::array<float, 3> p_i_perp;         // Perpendicular incoming POI polarization vector 
    std::array<float, 3> p_i_parr;         // Parallel incoming POI polarization vector 
    std::array<float, 3> S_r_norm;         // Normalized reflected Poynting vector
    std::array<float, 3> p_r_perp;         // Perpendicular reflected POI polarization vector 
    std::array<float, 3> p_r_parr;         // Parallel reflected POI polarization vector 
    std::array<float, 3> S_out_n;          // Container for Poynting-normal ext products
    std::array<float, 3> point;            // Point on target
    std::array<float, 3> norms;            // Normal vector at point
    std::array<float, 3> e_out_h_r;        // Real part of E-field - H-field ext product

    // Arrays of complex floats
    std::array<std::complex<float>, 3> e_r;            // Reflected E-field
    std::array<std::complex<float>, 3> h_r;            // Reflected H-field
    std::array<std::complex<float>, 3> n_out_e_i_r;    // Electric current
    std::array<std::complex<float>, 3> temp1;          // Temporary container 1 for intermediate irrelevant values
    std::array<std::complex<float>, 3> temp2;          // Temporary container 2
    
    // Return containers
    std::array<std::array<std::complex<float>, 3>, 2> beam_e_h; // Container for storing fieldAtPoint return
...
std::array<std::array<std::complex<float>, 3>, 2> Propagationf::fieldAtPoint(const std::vector<std::array<float, 3>> &grid_source,
                                               const std::vector<std::array<std::complex<float>, 3>> &Js,
                                               const std::vector<std::array<std::complex<float>, 3>> &Ms,
                                               ...
                                               const int start)
{
    ...
    float r_inv;                       // 1 / r
    float omega;                       // Angular frequency of field
    std::complex<float> Green;         // Container for Green's function
    std::complex<float> r_in_s;        // Container for inner products between wavevctor and currents
    
    // Arrays of floats
    std::array<float, 3> source_point; // Container for xyz co-ordinates
    std::array<float, 3> r_vec;        // Distance vector between source and target points
    std::array<float, 3> k_hat;        // Unit wavevctor
    std::array<float, 3> k_arr;        // Wavevector
    
    // Arrays of complex floats
    std::array<std::complex<float>, 3> e_field;        // Electric field on target
    std::array<std::complex<float>, 3> h_field;        // Magnetic field on target
    std::array<std::complex<float>, 3> js;             // Electric current at source point
    std::array<std::complex<float>, 3> ms;             // Magnetic current at source point
    std::array<std::complex<float>, 3> e_vec_thing;    // Electric current contribution to e-field
    std::array<std::complex<float>, 3> h_vec_thing;    // Magnetic current contribution to h-field
    std::array<std::complex<float>, 3> k_out_ms;       // Outer product between k and ms
    std::array<std::complex<float>, 3> k_out_js;       // Outer product between k and js
    std::array<std::complex<float>, 3> temp;           // Temporary container for intermediate values
    
    // Return container
    std::array<std::array<std::complex<float>, 3>, 2> e_h_field; // Return container containing e and h-fields
    ...
    for(int i=0; i<gridsize_s; i++)
    {
        ...
        Green = exp(this->t_direction * j * k * r) / (4 * M_PIf * r) * source_area[i] * j;
...
void Propagationf::parallelProp(const std::vector<std::array<float, 3>> &grid_target,
                               const std::vector<std::array<float, 3>> &grid_source,
                               const std::vector<std::array<float, 3>> &norm_target,
                               const std::vector<std::array<std::complex<float>, 3>> &Js,
                               const std::vector<std::array<std::complex<float>, 3>> &Ms,
                               const std::vector<float> &source_area)
{
    ...
    for(int n=0; n<numThreads; n++)
    {
        ...
        threadPool[n] = std::thread(&Propagationf::propagateBeam, 
...
void Propagationf::calculateFarField(int start, int stop,
                                ...
                                const std::vector<float> &source_area)
{
    ...
    std::complex<float> e_th;
    std::complex<float> e_ph;
    
    std::complex<float> e_Az;
    std::complex<float> e_El;

    // Arrays of floats
    std::array<float, 2> point_ff;            // Angular point on far-field
    std::array<float, 3> r_hat;                // Unit vector in far-field point direction

    // Arrays of complex floats
    std::array<std::complex<float>, 3> e;            // far-field E-field
...
std::array<std::complex<float>, 3> Propagationf::farfieldAtPoint(const std::vector<std::array<float, 3>> &grid_source,
                                               const std::vector<std::array<std::complex<float>, 3>> &Js,
                                               const std::vector<std::array<std::complex<float>, 3>> &Ms,
                                               const std::array<float, 3> &r_hat,
                                               const std::vector<float> &source_area,
                                               const int start)
{
    // Scalars (float & complex float)
    ...
    // Arrays of complex floats
    std::array<std::complex<float>, 3> e;        // Electric field on far-field point
    std::array<std::complex<float>, 3> _js;      // Temporary Electric current at source point
    std::array<std::complex<float>, 3> _ms;      // Temporary Magnetic current at source point
    
    std::array<std::complex<float>, 3> js;      // Build radiation integral
    std::array<std::complex<float>, 3> ms;      // Build radiation integral
    
    std::array<std::complex<float>, 3> _ctemp;
    std::array<std::complex<float>, 3> js_tot_factor;
    std::array<std::complex<float>, 3> ms_tot_factor;

    // Matrices
    std::array<std::array<float, 3>, 3> rr_dyad;       // Dyadic product between r_hat - r_hat
    std::array<std::array<float, 3>, 3> eye_min_rr;    // I - rr
    ...
    for(int i=0; i<gridsize_s; i++)
    {
        ...
        std::complex<float> expo = exp(j * k * r_hat_in_rp);
...
void Propagationf::parallelFarField(const std::vector<std::array<float, 2>> &grid_ff,
                               const std::vector<std::array<float, 3>> &grid_source,
                               const std::vector<std::array<std::complex<float>, 3>> &Js,
                               const std::vector<std::array<std::complex<float>, 3>> &Ms,
                               const std::vector<float> &source_area)
{
    ...
    for(int n=0; n<numThreads; n++)
    {
        ...
        threadPool[n] = std::thread(&Propagationf::calculateFarField, 
                                    this, n * step, final_step, 
                                    grid_ff, grid_source, Js, Ms, source_area);
    }
}

void Propagationf::joinThreads() 
{
