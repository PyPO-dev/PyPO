...
class Test_FitGauss(unittest.TestCase):
    ...
    def test_fitGauss(self):
        ...
        for mode in ["linear", "dB", "log"]:
            popt = self.s.fitGaussAbs(TestTemplates.GPOfield["name"], "Ex", thres=-11, mode=mode, full_output=True, ratio=None)
            self.assertTrue(len(popt) == 6)
            self.assertTrue(f"fitGauss_{TestTemplates.GPOfield['name']}" in self.s.scalarfields)

            popt = self.s.fitGaussAbs(TestTemplates.GPOfield["name"], "Ex", thres=-11, mode=mode, full_output=True, ratio=1)
            self.assertTrue(len(popt) == 6)
            self.assertTrue(f"fitGauss_{TestTemplates.GPOfield['name']}" in self.s.scalarfields)
